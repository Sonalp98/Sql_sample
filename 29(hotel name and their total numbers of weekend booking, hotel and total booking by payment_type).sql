...
from bookings
where dayname(booking_date)= 'Saturday' or 'Sunday'
group by hotel_name
order by total_bookings desc;
-- Your Task
-- Find out hotel_name and their total number of booking by credit card and cash
select hotel_name, 
sum(case when payment_type='Credit' then '1' else 0 end) credit_b,
sum(case when payment_type='Cash' then '1' else 0 end) cash_b
from bookings
group by hotel_name;
