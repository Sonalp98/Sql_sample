...
select count(*) as a ,round(count(*)*100.0/(select count(*) from fb_active_users
where country ='USA'),2) as share
from fb_active_users
where country = 'USA' and status = 'open';
--
select * ,active_users*100/total_users as share from 
(select sum(case when country = 'USA' and status = 'open' then 1 else 0 end) as active_users,
sum(case when country = 'USA'  then 1 else 0 end) as total_users
from fb_active_users) s;
--
select count(*) as total_users,
sum(case when country = 'USA' and status = 'open' then 1 else 0 end) as active_users,
round(sum(case when country = 'USA' and status = 'open' then 1 else 0 end)*100.0/count(*) ,2) as share
from fb_active_users
where country ='USA';
