...
with cte as(select city,round(avg(mkt_price)) as avg_price from house_price
group by city)
select id,state,h.city,mkt_price, avg_price
from house_price h
join cte c
on h.city= c.city
where mkt_price > avg_price
order by id;

select h1.id,h1.state,h1.city,h1.mkt_price
from house_price h1
where h1.mkt_price > (select avg(h2.mkt_price) as avg_price
                       from house_price h2
