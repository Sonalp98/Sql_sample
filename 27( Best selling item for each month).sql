...
with cte as(select month(invoicedate) as month, description, sum(quantity*unitprice) as amount_paid,
...
from cte
where rn=1;
-- Difference between two is first will get sum of all order for each product (casethere is a duplicate/multiple order)
select month(invoicedate) as month, description, (quantity*unitprice) as amount_paid,
dense_rank() over(partition by month(invoicedate) order by (quantity*unitprice) desc) as rn
from walmart_eu
group by month(invoicedate), description ,(quantity*unitprice);
select month(invoicedate) as month, description, sum(quantity*unitprice) as amount_paid,
dense_rank() over(partition by month(invoicedate) order by sum(quantity*unitprice) desc) as rn
from walmart_eu
group by month(invoicedate), description;
-- if these two entry for "LUNCH---" will be different then it would have appear in above query 
-- but it won't give overall sum for this product
select * from walmart_eu
where description = "LUNCH BAG SPACEBOY DESIGN";
