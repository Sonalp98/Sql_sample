...
select f1.* from forbes_global f1
where f1.profits = ( select max(profits) as max from forbes_global f2
                     where f2.country = f1.country)
order by country;
                     
select * from (select *, rank() over(partition by country order by profits desc) as rn
from forbes_global) s
where rn=1;