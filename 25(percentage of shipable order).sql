...
select Round(count(*)*100 /(select count(*) from orders),2) as pct
from orders o
left join customers c
on o.cust_id=c.id
where address is not null;
--
select Round(sum(case when address is not null then 1 else 0 end )*100/count(*),2) as Pct
from orders o
left join customers c
on o.cust_id=c.id;
-- This ans not good too lengthy
With cte as (select o.*, c.address 
from orders o
left join customers c
on  o.cust_id = c.id),
cte2 as (select * 
from cte 
where address is not null)
select count(*)*100/(select count(*) from cte) from cte2;
