...
order by user_id;
-- Approach with lead fun
with cte as(select *, lead(purchase_date,1) over(partition by user_id order by purchase_date) as nxt_p_date
from amazon_transactions)
select  distinct user_id from cte
where (nxt_p_date - purchase_date) <=7  and (nxt_p_date - purchase_date) <> 0 ;
...
and (t2.purchase_date - t1.purchase_date) <= 7
order by 1;
-- 
with cte as(select *, lead(purchase_date,1) over(partition by user_id order by purchase_date) as nxt_p_date
from amazon_transactions)
,cte2 as(
select * , coalesce((nxt_p_date - purchase_date),0) as diff from cte
where coalesce((nxt_p_date - purchase_date),0) <=7  and coalesce((nxt_p_date - purchase_date),0) <> 0 )
