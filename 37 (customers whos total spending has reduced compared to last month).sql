...
With cte as(select customer_name, s.customer_id, month(spending_date) as mon,
sum(spending_amount)  as spending  from spending_records s
left join customers c
on s.customer_id= c.customer_id
group by customer_name, s.customer_id,month(spending_date)),
cte2 as(
select customer_id,customer_name,
sum(case when mon = 3 then spending else 0 end )as previous_month,
sum(case when mon = 4 then spending else 0 end )as curr_month
from cte
group by customer_name, customer_id)
select * from cte2
where previous_month > curr_month ;
